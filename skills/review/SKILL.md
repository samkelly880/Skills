...
**Read-only means:** do not edit source files or the git worktree, and do not commit. The reviewer **must** still write its scratch artifacts and run `gh` / read-only git. **Do NOT pass `capability_mode: read-only`** when spawning it -- that mode removes the `write` tool, so the reviewer cannot produce `<review_file>` and the run dead-ends with findings it has no way to persist.
...
1. Confirm cwd is a git work tree: `git rev-parse --is-inside-work-tree`. If this fails, report that cwd is not a git repository and stop.
...
**Validate** that `REVIEW_ID` is 8 hex characters. If it is empty or the command failed, report the error (install Python 3) and stop.

**Inline the resolved absolute `scratch_dir` and `PYTHON`** into every later command and subagent prompt. Do not rely on a shell variable surviving across `run_terminal_command` calls.
...
2. Build a unified diff covering staged + unstaged tracked changes AND untracked files. Resolve `COLLECT_PY` as `<dirname of this SKILL.md>/scripts/collect_local_diff.py`. Run:

   ```
   <PYTHON> <COLLECT_PY> --diff <diff_file> --files <scratch_dir>/grok-review-files-<REVIEW_ID>.txt
   ```

   Parse the JSON stdout (`bytes`, `files`, `empty`). Do not use bash `read -d ''`, `/dev/null`, `wc -c`, or `sort -u` — those fail in PowerShell. Do not mutate the index.

   Trade-off note: `git diff --no-index` exits 1 when files differ. The collector treats that as success. Do not `git add -N`.

   **Fresh-repo guard**: on a brand-new `git init` with zero commits there is no `HEAD`. The collector leaves the tracked portion empty and captures the working tree via `git ls-files --others`.

   **Size gate (orchestrator-side)**: read the `bytes` field (or the written file size) and act on it:
...
3. Read `changed_files` from the collector's `files` array.
...
2. Fetch PR metadata in a single round trip (works for both numeric IDs and full URLs). Note that `gh pr view --json` does NOT expose a `baseRepository` field (verified: `gh pr view 1 --json baseRepository` returns `Unknown JSON field: "baseRepository"`. The available repo fields are `headRepository`, `headRepositoryOwner`, and `isCrossRepository`). The owner/repo for the upstream where the PR lives must therefore be derived from the `url` field instead. The `files` field is also requested here so we do not need a second round trip:
   ...
   - `changed_files` -- parse `.files[].path` from the prmeta JSON yourself (or `gh pr view` with `--jq ".files[].path"`). Do not invoke `jq` as a separate binary.
...
- Do **not** pass `capability_mode`. The reviewer needs `write` for `<review_file>`; the prompt below is what keeps it off source files.
...
That file is the only thing you write: never edit project source, never commit.
...
4. Build the JSON payload via the `write` tool, saving to `<pending_review_payload>`:
   ...
   **Construct the payload as structured data, then persist it with the `write` tool as valid JSON.** Do NOT concatenate JSON strings by hand and do not use a `python <<'PY'` heredoc (`<<` is reserved in PowerShell). Build the object, serialize it, and save it to `<pending_review_payload>`.
...
Cleanup: run `<PYTHON> <HOST_PY> cleanup` with the paths for the current mode/outcome (missing files are ok). Do not run `rm -f`.
...
- **Reviewer is read-only** -- the reviewer subagent must never modify source files (it only writes its review/diff/payload artifacts under the scratch dir). Enforce that through the prompt, never through `capability_mode`: `read-only` strips the `write` tool the reviewer needs for `<review_file>`. The orchestrator must never modify source files either. The only writes are to `<review_file>`, `<summary_file>`, `<diff_file>`, `<pending_review_payload>`, and the GitHub PENDING review.
...
- **Helpers are only for host work that is not portable in a shell** -- `shared/scripts/host.py` (scratch + id) and `scripts/collect_local_diff.py` (local untracked diffs). Review-file markdown parsing and the GitHub payload stay inline.
...
- **Host helpers, not review parsers.** `shared/scripts/host.py` and `scripts/collect_local_diff.py` exist so Setup and local diffs run on PowerShell. The markdown parse of `<review_file>` and the unified-diff hunk walk stay inline.
