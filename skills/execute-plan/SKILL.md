...
Run `<python> <HOST_PY> setup` and parse the JSON stdout. Store `python` as `PYTHON`, `scratch_dir` as `scratch_dir`, and `run_id` as `PLAN_ID`.

**Validate** that `PLAN_ID` is 8 hex characters. If it is empty or the command failed, report the error and stop.
...
**Invoke the helper from the workspace root** (the default cwd for `run_terminal_command`). The helper derives the workspace id from the cwd's git context.

### Read Path

1. Run `"${PYTHON}" "${MEMORY_HELPER}" snapshot` via `run_terminal_command` and capture stdout. The helper prints structured JSON:
...
Setup is skipped on resume, so first **recompute the scratch directory** — the state file and all per-PR artifacts live under it. Re-run `<PYTHON> <HOST_PY> scratch` so the path matches the original run's OS temp dir:
...
Then delete each per-PR summary and review file:

```
<PYTHON> <HOST_PY> cleanup <each grok-exec-summary-<PLAN_ID>-<pr-id>.md and grok-exec-review-<PLAN_ID>-<pr-id>.md you created>
```
...
```
<PYTHON> <MEMORY_HELPER> update --file <scratch_dir>/grok-exec-mem-<PLAN_ID>.json
```
...
```
<PYTHON> <HOST_PY> cleanup <scratch_dir>/grok-exec-mem-<PLAN_ID>.json
```
